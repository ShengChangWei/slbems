<template>
  <el-container>
    <el-aside width="25%">
      ...
      <div class="aside-content">
        <el-scrollbar style="height: 100%;">
          <el-tree :data="treeData" node-key="id" :current-node-key="resourceForm.id" default-expand-all draggable @node-drop="handleDrop" @node-drag-end="handleDragEnd" @node-contextmenu="showContextmenu" @node-click="treeClick">
            <span slot-scope="{ node, data }" class="custom-tree-node">
              <i class="icon" :class="data.icon"></i>
              <span class="label" :class="{'active': data.id===resourceForm.id}">{{ node.label }}</span>
              <span v-if="node.childNodes.length>0" class="floatRight number" :class="{'active': data.id===resourceForm.id}">{{ node.childNodes.length }}</span>
            </span>
          </el-tree>
        </el-scrollbar>
      </div>
    ...
    <el-dialog title="添加/编辑资源" :visible.sync="dialogFormVisible" width="30%">
      <el-form ref="dialogForm" :label-position="labelPosition" label-width="80px" :model="dialogForm">
        ...
        <el-form-item label="资源备注" prop="describe">
          <el-input v-model="dialogForm.describe" type="textarea" :rows="2" placeholder="请输入内容">
          </el-input>
        </el-form-item>
      </el-form>
